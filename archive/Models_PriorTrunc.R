...
beta0 ~ dnorm(0,0.1)T(-3,3) # Mean logit colonization probability (previously unoccupied sites)
beta1 ~ dnorm(0,0.1)T(-3,3) # Mean offset for persistence probability (previously occupied sites)

beta.QMD ~ dnorm(0,0.1)T(-3,3)
beta.EarlInf ~ dnorm(0,0.1)T(-3,3)
beta.MidInf ~ dnorm(0,0.1)T(-3,3)
beta.snag ~ dnorm(0,0.1)T(-3,3)
...
#save results
library(R.utils)
saveObject(bugout,"Mod_Pers_EInfMInfSnagQMD_PriorTrunc")
