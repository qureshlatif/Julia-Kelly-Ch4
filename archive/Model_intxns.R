...
parameters <- c("p","beta0","beta1","beta.EarlInf","beta.MidInf","beta.snag","beta.QMD","beta.EarlInfxQMD",
...
beta.EarlInfxQMD ~ dnorm(0,0.01)T(-30,30)
...
for(j in 1:nplot){  # Loop for year 1 - uses z0 to calculate persistence offset
  ...
  logit(psi[j,1]) <- beta0 + beta1*Z0[j] + beta.EarlInf*EarlInf.z[j,1] + beta.MidInf*MidInf.z[j,1] + 
    beta.snag*snag.z[j,1] + beta.QMD*QMD.z[j,1] + beta.EarlInfxQMD*EarlInf.z[j,1]*QMD.z[j,1]
...
saveObject(bugout,"Mod_Pers_QMDxEInf_MInfSnag")
