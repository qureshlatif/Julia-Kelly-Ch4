...
###---Simulate data based on fitted model parameter (median) estimates---###
library(R.utils)
...
truth <- c(beta0=0,beta.EarlInf=5,beta.MidInf=0,beta.QMD=0,beta.snag=0,p=0.5)

for(i in 1:dim(Y)[1]) {
  psi <- numeric(length=length(year))
  for(j in 1:dim(Y)[2]) psi[j] <- expit(truth["beta0"] + truth["beta.EarlInf"]*EarlInf.z[i,j] +
                 truth["beta.MidInf"]*MidInf.z[i,j] + truth["beta.QMD"]*QMD.z[i,j] +
                 truth["beta.snag"]*snag.z[i,j])
  z <- rbinom(length(psi),1,psi)
  Y[i,!is.na(Y[i,])] <- rbinom(sum(!is.na(Y[i,])),n.visits,z*truth["p"])
}
...
parameters <- c("p","beta0","beta.EarlInf","beta.MidInf","beta.snag","beta.QMD","test","psi.EI","psi.MI",
...
for(j in 1:nplot){  # Loop for year 1 - uses z0 to calculate persistence offset
  for(t in 1:nyear){  # Loop through remaining years 2 and up - uses Z[,,t-1] to calculate persistence offset
    ...
    logit(psi[j,t]) <- beta0 + beta.EarlInf*EarlInf.z[j,t] + beta.MidInf*MidInf.z[j,t] + 
...
#save results
library(R.utils)
#saveObject(bugout,"Mod_NoPers_EInfMInfSnagQMD_sim1")
#saveObject(bugout,"Mod_NoPers_EInfMInfSnagQMD_sim2")
saveObject(bugout,"Mod_NoPers_EInfMInfSnagQMD_sim3")
